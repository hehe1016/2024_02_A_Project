...
public class PlayerController : MonoBehaviour
{
    ...
    public float radius = 5.0f;                 //3인칭 카메[라와 플레이어 간의 거리
    ...
    void HandleRotation()
    {
        ...
        else
        {
            ...
            thirdPersonCamera.transform.LookAt(transform.position);     //LookAt 바라보는 방향 삼수 (캐릭터를 계속 본다)
    ...
    public void HandleMovement()
    {
        ...
        {
            Vector3 cameraFoward = thirdPersonCamera.transform.forward;   //카메라 앞 방향
            cameraFoward.y = 0.0f;
            cameraFoward.Normalize();   //방향 벡터 정규호하 (0~1)사이 값으로 만들어준다.
